...
class VentaDao extends FactoryBD implements DAO{
    ...
    public static function insert($objeto){
        $sql = 'insert into venta(fecha,cantidad,precio,producto,usuario values(?,?,?,?,?)';
        $objeto = (array)$objeto;
        $datos = array();
    ...
    public static function update($objeto){
        $sql = 'update venta set fecha = ?,cantidad = ?,precio = ?, producto = ?,usuario=? where id = ? ';
        $datos = array($objeto->fecha,$objeto->cantidad,$objeto->precio,$objeto->producto,$objeto->usuario,$objeto->id);
        $devuelve = parent::ejecuta($sql,$datos); 
        if($devuelve->rowCount() == 0){
            return false;
        }
