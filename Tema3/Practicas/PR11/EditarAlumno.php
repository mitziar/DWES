...
<body>
    ...
    <div class="jumbotron" style="background-color: bisque;">
        <div class="container">
            ...
            <h2>
                <center>FICHEROS CSV Y XML</center>
            </h2>
        </div>
    </div>
    ...
    <div class="container">
        <div class="row">
            <h4><a href="../../">Tema 3</a><a href="../">/Prácticas</a><a href='index.php'>/PR11</a>/EditarAlumno.php
            </h4>
        </div>
        <div class="row">
            <?php
        ...
        if (enviado() && validado()) {
            ...
            $dom->load('notas.xml'); //Cargo el xml en el objeto del DOM
        
            $raiz = $dom->childNodes[0]; //Accedo a los hijos del notas.xml, que será la raiz
        
            foreach ($raiz->childNodes as $alumnos) { //Recorro los hijos de la raiz que serán los alumnos del xml
                if ($alumnos->nodeType == 1) { //Si $alumno es un elemento nodo
                    foreach ($alumnos->childNodes as $datos) { //Por cada hijo del alumno, lo guardo en $datos
        
                        if ($datos->nodeType == 1 && $datos->nodeValue == $nombreAlumno) {
                            /**Si datos es nodeType Elemento y el valor de nodoElemento datos es igual al nombre del alumno */

                            $nuevo = $dom->createElement('nota1', $_REQUEST['nota1']);
                            /*Crea un elemento nuevo que está flotando*/
                            $datos = $datos->nextElementSibling;
                            if ($datos->nodeName == 'nota1') {
                                $alumnos->replaceChild($nuevo, $datos);
                                /*Lo remplazo con el creado, ya le doy posición en el dom*/
                            }
                            $datos = $datos->nextElementSibling;//Avanzo al siguiente hermanos
                            $nuevo = $dom->createElement('nota2', $_REQUEST['nota2']);

                            if ($datos->nodeName == 'nota2') {
                                $alumnos->replaceChild($nuevo, $datos);
                            }


                            $datos = $datos->nextElementSibling;
                            $nuevo = $dom->createElement('nota3', $_REQUEST['nota3']);
                            if ($datos->nodeName == 'nota3') {
                                $alumnos->replaceChild($nuevo, $datos);
                            }
